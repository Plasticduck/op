-- Daily summary data for the 7:40am Central digest email (Mighty Wash).
-- Returns one JSON blob with the reporting day's totals, comparisons, per-site
-- breakdown, month-to-date, and the latest monthly membership metrics. The
-- reporting day is the last complete day (yesterday in Central), since today's
-- numbers are still filling in when the email goes out.
...
revoke all on function public.mw_daily_summary() from public, authenticated;
grant execute on function public.mw_daily_summary() to service_role;
