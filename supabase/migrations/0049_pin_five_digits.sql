-- 0049_pin_five_digits.sql
-- Kiosk PINs are now 5 digits instead of 4. Redefines the three PIN functions
-- (exact current bodies) with the length check changed from \d{4} to \d{5}.
-- NOTE: employees with an existing 4-digit PIN must have a new 5-digit PIN set;
-- their old PIN can no longer be entered at the 5-digit kiosk pad.
...
begin
  ...
  if p_pin !~ '^\d{5}$' then raise exception 'PIN must be exactly 5 digits'; end if;
  if exists (
