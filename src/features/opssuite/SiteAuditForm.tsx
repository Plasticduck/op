              ...
              <Field key={item.id} label={item.label}>
                {(id) => (
                  <div className="flex flex-col gap-1">
                    {item.helpText && (
                      <p className="text-xs text-ink-muted">{item.helpText}</p>
                    )}
                    {renderControl(id, item, (answers[item.id] as Record<string, unknown> | undefined) ?? {}, setItem)}
                  </div>
                )}
              </Field>
            ))}
    ...
    case 'attachment': {
      return (
        <p className="text-sm italic text-ink-muted">
          Attach files after saving the audit.
        </p>
      )
    }
    case 'comments': {
