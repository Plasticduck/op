...
import {
  ...
  type SiteAuditSection,
} from './siteAuditSchema'

const ITEM_TYPES: { value: SiteAuditItemType; label: string }[] = [
...
export function SiteAuditBuilder({
  open,
  onClose,
  onSaved,
}: {
  ...
  const [draft, setDraft] = useState<SiteAuditSchema>(DEFAULT_SITE_AUDIT_SCHEMA)
  ...
  useEffect(() => {
    ...
    void customForms
      .get('site_audit')
      .then(({ data }) => {
        const schema = data?.schema as SiteAuditSchema | undefined
        setDraft(schema?.sections?.length ? schema : DEFAULT_SITE_AUDIT_SCHEMA)
      })
      .then(() => setLoading(false), () => setLoading(false))
  }, [open])

  const updateSection = (idx: number, patch: Partial<SiteAuditSection>) => {
    setDraft((d) => ({
      ...d,
  ...
  const updateItem = (sIdx: number, iIdx: number, patch: Partial<SiteAuditItem>) => {
    setDraft((d) => ({
      ...d,
  ...
  const save = async () => {
    ...
    const normalized: SiteAuditSchema = {
      version: 1,
  ...
  return (
    <Modal open={open} onClose={onClose} title="Customize Site Audit" size="lg">
      {loading ? (
        <p className="text-sm text-ink-muted">Loading.</p>
      ) : (
        <div className="flex flex-col gap-4">
          <p className="text-sm text-ink-muted">
            Build the checklist your team fills out for each audit. Reorder sections and items with the arrows.
          </p>
                    ...
                    >
                      ...
                      <div className="w-44">
                        <Select
                          value={item.type}
                          onChange={(e) =>
                            updateItem(sIdx, iIdx, { type: e.target.value as SiteAuditItemType })
                          }
                        >
                          {ITEM_TYPES.map((t) => (
